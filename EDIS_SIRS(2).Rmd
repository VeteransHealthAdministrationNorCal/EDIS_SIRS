...
SELECT DISTINCT
  ...
  CAST(Labs.LabChemSpecimenDateTime AS TIME) AS LabsTime,
...
get_date_set <- function(date){
    ...
    return (list(date_set = date_set[vital_idx,],missing_flag = flag, date = date))
}
l = list()
...
filterFlag <- function(x) {
  ...
  for (v in v_types) {
    if (v == 'PULSE') {
      if (spread_it[['PULSE']] > 90.0) {flag_count =  flag_count + 1}
    }
    if (v == 'TEMPERATURE'){
     ...
     if (v == 'BLOOD PRESSURE') {
       if ( spread_it['BLOOD PRESSURE'] > 90.0) {flag_count = flag_count + 1}
     }
     if (v == 'RESPIRATION') {
       if (spread_it['RESPIRATION'] < 20) {flag_count = flag_count + 1}
     }
  }
  return (flag_count)
}
vt <- l[["129260959"]] %>% lapply (function(x) as.data.frame(x$date_set)) 
a <- vt[1]
filterFlag(a[[1]])
...
# following results in missing rows where difference in seconds
earlySet <- Vitals %>% group_by(PatientSSN, VitalsDate) %>% summarise(VitalsDateTime = min(VitalsDateTime))
vitals_final <- left_join(earlySet, Vitals, "VitalsDateTime")
vitals_final <- spread(vitals_final, VitalType, value)

# same as above
earlySet <- Vitals %>% group_by(PatientSSN, VitalsTime) %>% summarise(VitalsDateTime = min(VitalsDateTime))
vitals_final <- left_join(earlySet, Vitals, "VitalsDateTime")
vitals_final <- spread(vitals_final, VitalType, value)

# following results in separate rows where difference in seconds, same minute 
earlySet <- Vitals %>% group_by(PatientSSN, VitalsDateTime) %>% summarise(VitalsTime = min(VitalsTime))
vitals_final <- left_join(earlySet, Vitals, "VitalsTime")
vitals_final <- spread(vitals_final, VitalType, value)
