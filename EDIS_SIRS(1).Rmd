...
#Vitals$ArrivalDate <- Vitals$ArrivalDate %>% sapply(function(x) strftime(x,'%Y-%m-%d'))
Vitals$VitalsTime <- Vitals$VitalsDateTime  %>% sapply(function(x) strftime(x,'%Y-%m-%d %H:%M'))
Vitals <- select(Vitals, -Systolic,-Result)
earlySet <- Vitals %>%  summarise(VitalsDateTime = min(VitalsDateTime))
#earlySet <- Vitals %>% group_by(VitalsDateTime) %>% summarise(VitalsDateTime = min(VitalsDateTime))

VitalFirstTest <- Vitals %>% inner_join(earlySet)
...
for (j in 1:length(labList)) {
  line <- rbind(line, labList[[j]])
}

for (k in 1:length(labList)) {
  labList[[k]] <- labList[[k]][1]
}
